...
class HygieneDetails extends StatefulWidget {
  String detailof;
  @override
  _HygieneDetailsState createState() => _HygieneDetailsState(this.detailof);

  HygieneDetails(str){this.detailof=str;}

}

class _HygieneDetailsState extends State<HygieneDetails> {
  String type;
  _HygieneDetailsState(str){
    this.type=str;
    _getduration();
    _getdetails();
  }
  ...
  _getdetails() async {
    ...
    final msg = jsonEncode({"email": user, "type": type});
...
var nextdatetime;
  _addnew() async{
    ...
    final msg = jsonEncode({"email": user, "type": type,"nexttime":nextdatetime.toString(),"time":DateTime.now().toString()});
  ...
  _getduration() async{
    ...
    final msg = jsonEncode({"type": type});
    Response response = await post(url, headers: headers, body: msg);
    int statusCode = response.statusCode;
    if (statusCode == 200) {
      String bodyText = response.body;
                  ...
                  Padding(
                    ...
                    child: GestureDetector(
                      child:  ListTile(
                        ...
                        title: Text('Schedule'),
                        subtitle: Text("Repeat after every"),
                        trailing:Text("$timeperiod $durationtype"),
                      ),
                      onTap: () async{
//                        await _asyncInputDialog(context);
//                        await _displayDialog2(context);
//                        await _update();
                  ...
                  Padding(
                    ...
                    child: (data!=null)? ListTile(
                      ...
                      title: Text('Upcoming'),
                      subtitle:Text('Next time for $type \n$nexttime\non $nextdate'),
